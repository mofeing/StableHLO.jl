...
```
"""
function all_gather(operand::Value; result_0::IR.Type, all_gather_dim, replica_groups, channel_handle=nothing, use_global_device_ids=nothing, location=Location())
...
```
"""
function all_reduce(operand::Value; result_0::IR.Type, replica_groups, channel_handle=nothing, use_global_device_ids=nothing, computation::Region, location=Location())
    results = IR.Type[result_0, ]
    operands = Value[operand, ]
...
```
"""
function all_to_all(operand::Value; result_0=nothing::Union{Nothing, IR.Type}, split_dimension, concat_dimension, split_count, replica_groups, channel_handle=nothing, location=Location())
    results = IR.Type[]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
function batch_norm_grad(operand::Value, scale::Value, mean::Value, variance::Value, grad_output::Value; grad_operand=nothing::Union{Nothing, IR.Type}, grad_scale=nothing::Union{Nothing, IR.Type}, grad_offset=nothing::Union{Nothing, IR.Type}, epsilon, feature_index, location=Location())
    ...
    !isnothing(grad_operand) && push!(results, grad_operand)
    !isnothing(grad_scale) && push!(results, grad_scale)
    !isnothing(grad_offset) && push!(results, grad_offset)
...
function batch_norm_training(operand::Value, scale::Value, offset::Value; output=nothing::Union{Nothing, IR.Type}, batch_mean=nothing::Union{Nothing, IR.Type}, batch_var=nothing::Union{Nothing, IR.Type}, epsilon, feature_index, location=Location())
    ...
    !isnothing(batch_mean) && push!(results, batch_mean)
    !isnothing(batch_var) && push!(results, batch_var)
...
```
"""
function var"cross-replica-sum"(operand::Value; result_0=nothing::Union{Nothing, IR.Type}, replica_groups, location=Location())
    results = IR.Type[]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function dot_general(lhs::Value, rhs::Value; result_0::IR.Type, dot_dimension_numbers, precision_config=nothing, location=Location())
    results = IR.Type[result_0, ]
    operands = Value[lhs, rhs, ]
    owned_regions = Region[]
    successors = Block[]
